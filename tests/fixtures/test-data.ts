...
export const createMultiLineOutput = (lineCount: number, lineContent: string = 'Test line'): string => {
  return Array.from({ length: lineCount }, (_, i) => `${lineContent} ${i + 1}`).join('\n');
};

// Formatted command output (matches MCP server format)
export const formatCommandOutput = (stdout: string, stderr: string, returnCode: number): string => {
